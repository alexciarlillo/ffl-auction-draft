...
class DB {
  static async createLobby({ name, franchiseCount, franchiseBudget }) {
    ...
    try {
      lobby = await db.one(
        "insert into lobbies(name, franchise_count, franchise_budget) values(${name}, ${franchiseCount}, ${franchiseBudget}) RETURNING id",
        { name, franchiseCount, franchiseBudget }
      );
    } catch (err) {
      console.log(err);
    } finally {
  ...
  static async getFranchisesForLobby(lobbyId) {
    let franchises = null;
    try {
      franchises = await db.many(
        "SELECT * FROM franchises WHERE lobby_id = ${lobbyId}",
        { lobbyId }
      );
    } catch (err) {
      console.log(err);
  ...
  static async claimFranchise({ franchiseId }) {
    return await db.one(
      "UPDATE franchises SET claimed_at = now() WHERE id = ${franchiseId}",
      { franchiseId }
    );
  }
